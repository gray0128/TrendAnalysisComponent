...
```bash
npm install @gray0128/trend-analysis-component@0.1.5
```
...
`diagnoseBaseUrl` 只传诊断分析应用基址，不要包含 `#/comprehensiveTrend/...`。0.1.5 会从应用根入口跳转，将 `origin`、`deviceCode` 等查询参数放在 Hash 路由之前。多选数据项时，组件同时写入 `sessionStorage.deviceKpi`，供诊断分析页恢复选中项并自动查询。
